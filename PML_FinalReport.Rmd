...
To predict all 20 observations with 95% confidence, I _conservatively_ estimate a final model will need to have an accuracy of $1 - \frac{0.05}{20}$ = `r round(1- 0.05/20, 4)` (bonferroni correction). This essentially means the final out-of-sample error rate must be less than `r round(0.05/20, 4)` ($1 - accuracy$).

> To learn more about the estimate see [FWER - Controlling Procedures](https://en.wikipedia.org/wiki/Family-wise_error_rate#Controlling_procedures).
...
At first glance, I discovered the presence of what I call "metafeatures" (_i.e._ features that describe how the data was collected, such as `user_name`, `num_window`, and `cvtd_timestamp`). Because any individual participant can only complete one activity at a time, it seems reasonable to guess that I can train a model with high accuracy based solely on metafeatures. For curiosity's sake, I'll train two different models: the [first](#model1) on metafeatures, and the [second](#model2) strictly with sensor measurements which is what I would expect to use in a real world application.

# Cross validation & Out-of-sample error procedures
For both models I will use 10-fold cross-validation for training the model based on the recommendation of [Max Kuhn](http://appliedpredictivemodeling.com/blog/2014/11/27/vpuig01pqbklmi72b8lcl3ij5hj2qm) and then will estimate out-of-sample accuracy/error using 30% of the data provided.

# Model 1 - Training on Metafeatures {#model1}
Combining `user_name` with at least one time-based variable will likely be enough to predict `classe` with high accuracy. The `cvtd_timestamp` values are probably too homogeneous (only `r wl_train$cvtd_timestamp %>% unique() %>% length()` unique values). The raw timestamps combined (now `timestamp`) provide a unique value for each observation and appear to provide sufficient separation to build a model. I will train a random forest algorithm because they generally perform well on classification problems out of the box.
...
The resulting model is extremely accurate but doesn't quite reach the desired accuracy threshold. Including 1 or more additional "metafeatures" I expect would put the model over that threshold.
...
Among the remaining "metafeatures", `num_window` has the most unique values so it will probably do the job.
...
```{r time_importance, include = FALSE}
...
```
While those 100 features may be useful for application in a real world context (a partial goal of this model), they will probably not be useful for predicting the 20 quiz observations (still a goal for this model). Also, missing values are problematic for machine learning algorithms and these are not easily imputed in a meaningful way (maybe the easiest approach would be duplication but would it be helpful?). The best approach, then, for training a real world model (that still works for the quiz) is to remove features with missing data along with the metafeatures. That leaves the data set with 52 features to train on.
...
I expect a random forest algorithm would again perform best here but I'll compare that against multinomial logistic regression, linear discriminant analysis (LDA), and step-wise LDA.
...
```{r real_models}
snsr_rf <- train(classe ~ ., method = "rf",
                    data = snsr_train,
                    trControl = fitControl
                    )
...
```{r snsr_oob}
oob_accuracy(snsr_test, classe, snsr_rf, snsr_mlr, snsr_lda, snsr_slda)
```
I'm impressed that it achieved an accuracy that was close to that of the 3-metafeature random forest. Like that model it correctly predicted all 20 observations on the final quiz, while the other algorithms would not have obtained a passing grade (`snsr_mlr` = 15 correct, `snsr_lda` = 14, and `snsr_slda` = 9).
